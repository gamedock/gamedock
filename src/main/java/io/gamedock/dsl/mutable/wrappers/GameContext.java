...
import io.gamedock.dsl.mutable.managers.MetricsManager;
...
public class GameContext {
    ...
    public GameContext(Game game, ActivityContext activity) {
        this.game = game;
    ...
    public List<TeamContext> getTeams() {
        return Collections.unmodifiableList(teams.stream().map(t -> new TeamContext(t, activity)).collect(Collectors.toList()));
    }

    public List<PlayerContext> getPlayers() {
        return Collections.unmodifiableList(players.stream().map(m -> new PlayerContext(m, activity)).collect(Collectors.toList()));
    }

    public MetricsManager getMetrics() {
        return new MetricsManager(game, activity);
    }

    public List<CharacterContext> getCharacters() {
        return Collections.unmodifiableList(characters.stream().map(p -> new CharacterContext(p, activity)).collect(Collectors.toList()));
    }

    public List<PlayContext> getPlays() {
        return Collections.unmodifiableList(plays.stream().map(p -> new PlayContext(p, activity)).collect(Collectors.toList()));
    }
