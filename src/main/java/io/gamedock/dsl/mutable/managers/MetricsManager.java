...
import io.gamedock.dsl.mutable.wrappers.PointMetricContext;
import io.gamedock.dsl.mutable.wrappers.SetMetricContext;
import io.gamedock.dsl.mutable.wrappers.StateMetricContext;
...
public class MetricsManager {
    ...
    public MetricsManager(Game game, ActivityContext activity) {
    ...
    public PointMetricContext points(String name) {
        return points.stream().filter(m -> m.getName().equals(name)).findFirst().map(m -> new PointMetricContext(m, activity)).orElse(null);
    }

    public List<PointMetricContext> getPoints() {
        return Collections.unmodifiableList(points.stream().map(m -> new PointMetricContext(m, activity)).collect(Collectors.toList()));
    }

    public SetMetricContext sets(String name) {
        return sets.stream().filter(m -> m.getName().equals(name)).findFirst().map(m -> new SetMetricContext(m, activity)).orElse(null);
    }

    public List<SetMetricContext> getSets() {
        return Collections.unmodifiableList(sets.stream().map(m -> new SetMetricContext(m, activity)).collect(Collectors.toList()));
    }

    public StateMetricContext states(String name) {
        return states.stream().filter(m -> m.getName().equals(name)).findFirst().map(m -> new StateMetricContext(m, activity)).orElse(null);
    }

    public List<StateMetricContext> getStates() {
        return Collections.unmodifiableList(states.stream().map(m -> new StateMetricContext(m, activity)).collect(Collectors.toList()));
    }
