...
public class SetScoreContext {
    ...
    public SetScoreContext(SetScore score) {
        this.score = score;
    ...
    public SetItemScoreContext items(String name) {
        return itemScores.stream().filter(i -> i.getItem().getName().equals(name)).findFirst().map(i -> new SetItemScoreContext(i)).orElse(null);
    }

    public List<SetItemScoreContext> getItems() {
        return Collections.unmodifiableList(itemScores.stream().map(i -> new SetItemScoreContext(i)).collect(Collectors.toList()));
    }
    ...
    public SetMetricContext getMetric() {
        return new SetMetricContext(metric);
    }
